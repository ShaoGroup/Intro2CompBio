...
# Hierarchical clustering using data processed by PCA
  I realize the PCA algorithm and compress the initial data according to 'rerr'(here 'rerr' equals 0.1) which means how much information will be thrown away after PCA. The compressed data shows there are 102 principal components extracted. 
...
# discussion
  In the final of this procedure I count all the mis_classificated samples. And we can see the error classification rate decreases(from 0.086 to 0.064) if we compress the data using PCA, which proves the correctness of the classification in turn.
